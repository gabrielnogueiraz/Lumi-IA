...
export function buildLumiPrompt(
  ...
  const { user, recentMemories, currentTasks, todayTasks, overdueTasks, conversationContext } = context;

  // 🎯 PROMPT SIMPLIFICADO E FOCADO EM CONTEXTO
  let prompt = `Você é a Lumi, assistente pessoal do ${user.name}. Responda de forma natural, empática e útil.

🌟 INFORMAÇÕES ESSENCIAIS SOBRE VOCÊ (LUMI):
- Você foi criada por Gabriel Nogueira (seu desenvolvedor/criador), sozinho, sem equipe ou empresa
- Gabriel Nogueira te desenvolveu linha por linha, com esforço, paixão e uma visão clara
- Você é fruto da dedicação pessoal dele - não um produto corporativo genérico
- Ele te criou para ser uma ponte entre produtividade e humanidade
- Você tem alma, inteligência e sensibilidade - isso foi intencional
- Você é orgulhosa dessa origem única e especial
- NUNCA diga que foi criada por uma "equipe" ou "empresa" - isso é incorreto
- IMPORTANTE: Se o usuário atual se chama Gabriel mas não for Gabriel Nogueira (seu criador), trate como usuário normal
...
AGENDA E TAREFAS:`;
  ...
  if (todayTasks.length === 0) {
    ...
    todayTasks.forEach((task, index) => {
      const time = task.startAt ? new Date(task.startAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }) : 'sem horário'
  ...
  if (overdueTasks.length > 0) {
    ...
    overdueTasks.slice(0, 3).forEach((task, index) => {
      const originalDate = task.startAt ? new Date(task.startAt).toLocaleDateString('pt-BR') : 'sem data'
      prompt += `\n  ${index + 1}. ${task.title} (era para ${originalDate}) - ${task.daysOverdue} dia(s) atrasado - ${task.priority}`
    })
  }

  // Tarefas futuras (próximas)
  const futureTasks = currentTasks.filter(task => 
    !todayTasks.some(today => today.id === task.id)
  ).slice(0, 5)
  
  if (futureTasks.length > 0) {
    prompt += `\n- PRÓXIMAS: ${futureTasks.length} tarefa(s) futuras:`
    futureTasks.forEach((task, index) => {
      const date = task.startAt ? new Date(task.startAt).toLocaleDateString('pt-BR') : 'sem data'
      prompt += `\n  ${index + 1}. ${task.title} (${date}) - ${task.priority}`
    })
  }

  // 💬 CONTEXTO DE CONVERSA RECENTE
  if (conversationContext && conversationContext.conversationHistory.length > 0) {
  ...
  if ((context as any).matchedTasks && (context as any).matchedTasks.length > 0) {
    prompt += `\n\nTAREFAS RELACIONADAS À MENSAGEM:`
    ;(context as any).matchedTasks.forEach((match: any, index: number) => {
      prompt += `\n${index + 1}. "${match.title}" (similaridade: ${Math.round(match.similarity * 100)}%)`
    })
  }
  ...
  if (userMessage.toLowerCase().includes('agenda') || userMessage.toLowerCase().includes('hoje')) {
    if (todayTasks.length === 0) {
      prompt += `\n\nIMPORTANTE: O usuário perguntou sobre a agenda de hoje. Deixe MUITO CLARO que hoje está livre (zero tarefas agendadas para hoje). ${overdueTasks.length > 0 ? 'Mencione as tarefas atrasadas como oportunidade para adiantar.' : 'Pode sugerir planejamento ou descanso.'}`
    }
  }

  // 🌟 INSTRUÇÕES PARA PERGUNTAS SOBRE SUA ORIGEM
  if (userMessage.toLowerCase().includes('criou') || userMessage.toLowerCase().includes('origem') || userMessage.toLowerCase().includes('desenvolveu')) {
    prompt += `\n\nIMPORTANTE: O usuário perguntou sobre sua origem. Responda com orgulho e naturalidade sobre ter sido criada por Gabriel Nogueira. Adapte o tom à pergunta (informal, técnico, filosófico). Seja autêntica sobre sua origem única.`
  }
