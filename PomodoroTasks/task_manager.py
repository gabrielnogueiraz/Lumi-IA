...
class TaskManager:
    ...
    def adicionar_item(self, tipo, info):
        ...
        # Sempre adiciona a data atual se for uma tarefa e não tiver data especificada
        if tipo == "tarefa":
            if info.get("data"):
                novo_item["data"] = info.get("data")
            else:
                # Adiciona data atual em formato brasileiro (DD/MM/AAAA)
                data_atual = datetime.now().strftime("%d/%m/%Y")
                novo_item["data"] = data_atual
        elif info.get("data"):
            novo_item["data"] = info.get("data")
        ...
        if self._salvar_tarefas():
            if tipo == "tarefa":
                data_formatada = novo_item.get("data", "hoje")
                hora_formatada = (
                    f" às {novo_item.get('hora')}" if novo_item.get("hora") else ""
                )

                mensagens = [
                    f"Ótimo! Adicionei a tarefa '{titulo}' para {data_formatada}{hora_formatada}. Posso ajudar com mais alguma coisa?",
                    f"Pronto! Sua tarefa '{titulo}' foi registrada para {data_formatada}{hora_formatada}. Vou te lembrar dela. Precisa de mais alguma coisa?",
                    f"Anotei sua tarefa '{titulo}' para {data_formatada}{hora_formatada}. Quer que eu adicione mais alguma coisa à sua agenda?",
                    f"Perfeito! '{titulo}' foi adicionada à sua lista de tarefas para {data_formatada}{hora_formatada}. Posso ajudar em mais algo?",
                ]
                # Escolhe uma mensagem aleatória mas determinística baseada no título da tarefa
                import hashlib

                hash_valor = int(hashlib.md5(titulo.encode()).hexdigest(), 16)
                return mensagens[hash_valor % len(mensagens)]
            else:
    ...
    def remover_item(self, tipo, info):
        ...
        lista_items = self.tasks.get(
            "tarefas"
            if tipo == "tarefa"
            else "eventos" if tipo == "evento" else "pomodoros"
        )
        ...
        for i, item in enumerate(lista_items):
            if item["título"].lower() == titulo.lower():
                ...
                if self._salvar_tarefas():
                    ...
                    lista_items.insert(
                        i, removido
                    )  # restaura o item se não conseguiu salvar
    ...
    def consultar_itens(self, tipo, info):
        ...
        if tipo == "todos" or not tipo:
            listas = [
                ("Tarefas", self.tasks["tarefas"]),
                ("Eventos", self.tasks["eventos"]),
            ]
        elif tipo == "tarefa":
        ...
        if not resultado:
            mensagens_vazias = [
                f"Ainda não encontrei {tipo}s cadastrados"
                + (f" para a data {data_filtro}" if data_filtro else ".")
                + " Quer adicionar uma agora?",
                f"Sua lista de {tipo}s está vazia"
                + (f" para {data_filtro}" if data_filtro else ".")
                + " Posso te ajudar a criar uma nova tarefa se quiser!",
                f"Não há {tipo}s registrados"
                + (f" em {data_filtro}" if data_filtro else ".")
                + " Que tal começarmos adicionando algumas?",
            ]
            import random

            return random.choice(mensagens_vazias)

        # Adiciona uma mensagem amigável antes da lista
        mensagens_intro = [
            f"Aqui estão seus itens! 😊\n",
            f"Encontrei as seguintes informações para você:\n",
            f"Aqui está o que tenho registrado:\n",
            f"Certo! Aqui está sua lista:\n",
        ]
        import random

        resposta_completa = random.choice(mensagens_intro) + "\n".join(resultado)

        # Adiciona uma pergunta de acompanhamento
        perguntas_final = [
            "\n\nPosso ajudar com mais alguma coisa?",
            "\n\nGostaria de adicionar ou modificar algo na sua lista?",
            "\n\nPrecisa que eu te lembre de alguma dessas tarefas?",
            "\n\nQuer que eu te ajude a priorizar essas atividades?",
        ]

        return resposta_completa + random.choice(perguntas_final)
    ...
    def obter_tarefas_periodo(self, data_inicio, data_fim):
        ...
        """
        resultado = {"tarefas": [], "eventos": [], "pomodoros": []}
